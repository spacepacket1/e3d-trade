...
AGENT="harvest"
CONFIG="${TRAIN_DIR}/train_config_harvest_v1.yaml"
ADAPTER_DIR="adapters_harvest_v1"
DATA_DIR="data/harvest"
...
python3 "${TRAIN_DIR}/extract_agent_training_data.py" --agent harvest --output data --synthetic-count 300
...
if [[ "${NEW_LOSS}" != "null" && -f "${RUNS_LOG}" ]]; then
  # Find last harvest entry with status=ok, extract eval_loss
  PREV_LOSS=$(grep '"agent": *"harvest"' "${RUNS_LOG}" | grep '"status": *"ok"' | \
    python3 -c "
import sys, json
entries = [json.loads(l) for l in sys.stdin if l.strip()]
ok = [e for e in entries if e.get('agent') == 'harvest' and e.get('status') == 'ok']
if ok:
...
ADAPTER_VERSION="harvest_v1_$(date +%Y%m%d)"
...
if [[ "${STATUS}" == "rolled_back" ]]; then
  log "Harvest training FAILED — adapter rolled back."
  exit 1
fi

log "Harvest training complete."
